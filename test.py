...
parser.add_argument('-num_bins', default=2, help='number of pressure bins to use', type=int) # that sets the default argument to 2
...
df['CTDPRS'] = df['CTDPRS'].mask(df['CTDPRS'] > args.maxp, np.nan)
...
for c in range(1, max(df['CRUISENO'])):
  ...
  for b in range(args.num_bins):
    bin = cruise.loc[cruise['pbin'] == b].drop(columns=['pbin']).mean(skipna=True).add_suffix(f"bin{b}")
